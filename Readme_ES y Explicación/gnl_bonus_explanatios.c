...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bonus_gnl_explanatio.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ravazque <ravazque@student.42madrid.com    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/23 13:19:48 by ravazque          #+#    #+#             */
/*   Updated: 2024/10/23 22:05:13 by ravazque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*extract_line(char *buffer)
{
	...
	while (buffer[i] && buffer[i] != '\n')						// avanzo para calcular el tamaño de la línea a devolver
	{
		i++;
	}
...
char	*get_next_line(int fd)
{
	...
	while(!ft_strchr(buffer[fd], '\n') && bytes_read > 0)	 	// si no hay saltos de líneas y la cantidad de contenido leído es mayor que 0 entonces ->
	{
		...
		buffer[fd] = ft_strjoin(buffer[fd], tem_buffer);		// concateno en mi buffer estático, con su correspondiente fd, el contenido del buffer temporal
			if (!buffer[fd])
			{
				free (tem_buffer);
				return (NULL);
	...
	free (tem_buffer);											// libero el buffer temporal ya que ya tengo el contenido del fd guardado en "buffer[fd]"
	...
	line = extract_line(buffer[fd]);							// mando el buffer con el contenido para que me extraiga los primeros caracteres hasta encontrar un '\n'
	
	buffer[fd] = upd_buffer(buffer[fd]);						// mando el buffer a la función para quitar toda la línea que ya se ha guardado en line
	
	return(line);												// devuelvo la línea correspondiente
}
